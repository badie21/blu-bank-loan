...
const Calendar: React.FC<Props> = ({
  ...
  const getCalendarBlockDetails = (year: number, month: number) => {
    ...
    if (offsetFromPrevMonth > 0) {
      const prevMonthDaysCount = month === 1 ? getNumberOfDays(year - 1, 12, locale) : getNumberOfDays(year, month - 1, locale);
      ...
      for (let offset = 0; offset < offsetFromPrevMonth; offset++) {
        ...
        const date = new Date(locale === 'fa' ? jmoment(dateString, 'jYYYY-jMM-jDD').format() : dateString);
  ...
  const renderDay = useCallback(
    (day: { currentMonth: boolean; timestamp: number }, index: number): React.ReactNode => {
      ...
      const isSelectedSingleDate = isEqualDays(day.timestamp, startDate) && type === 'single';

      const isHoveredDay = startDate && !endDate && (state.hoveredDay as number) >= day.timestamp && day.timestamp > startDate;

      const isFromDate = isEqualDays(day.timestamp, startDate) && type === 'range';

      const isToDate = type === 'range' && day.timestamp === endDate;

      const isInrange = endDate && startDate && day.timestamp > startDate && day.timestamp < endDate;
      ...
      return (
        ...
        >
          <span>{locale === 'fa' ? currentDay.jDate().toLocaleString('fa') : currentDay.date()}</span>
        </div>
      );
    },
  ...
  const renderCalendar = (year: number, month: number) => {
    ...
    return (
      <>
        <div className={styles['month__header']} style={{ flexDirection: locale === 'fa' ? 'row' : 'row-reverse' }}>
        ...
        <div className={styles['month__body']} style={{ flexDirection: locale === 'fa' ? 'row' : 'row-reverse' }}>
          {days}
        </div>
      </>
    );
  ...
  return (
    <div className={classNames(styles['calendar__container'], containerClassName)}>
